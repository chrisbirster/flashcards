...
const FLAG_COLORS = [
  {
    ...
    name: "None",
    color:
      "bg-[var(--app-card-strong)] text-[var(--app-text-soft)] border-[var(--app-line-strong)]",
  },
...
type StudySessionStatus = "active" | "completed" | "abandoned";
...
const EMPTY_STUDY_SESSION_PROGRESS: StudySessionProgress = {
  cardsReviewed: 0,
  againCount: 0,
  hardCount: 0,
  goodCount: 0,
...
function nextStudySessionProgress(
  progress: StudySessionProgress,
  rating: number,
): StudySessionProgress {
  return {
    cardsReviewed: progress.cardsReviewed + 1,
    againCount: progress.againCount + (rating === 1 ? 1 : 0),
    hardCount: progress.hardCount + (rating === 2 ? 1 : 0),
    goodCount: progress.goodCount + (rating === 3 ? 1 : 0),
    easyCount: progress.easyCount + (rating === 4 ? 1 : 0),
  };
}
...
export function StudyScreen({ deckId, deckName, onExit }: StudyScreenProps) {
  ...
  const [studySessionStatus, setStudySessionStatus] =
    useState<StudySessionStatus | null>(null);
  const [sessionProgress, setSessionProgress] = useState<StudySessionProgress>(
    EMPTY_STUDY_SESSION_PROGRESS,
  );
  const questionStartTimeRef = useRef(0);
  const pendingSessionBootstrapRef = useRef<Promise<string | null> | null>(
    null,
  );
  ...
  const createStudySessionMutation = useMutation({
    mutationFn: () => repository.createStudySession({ deckId, mode: "review" }),
  });
  ...
  const updateStudySessionMutation = useMutation({
    mutationFn: ({
      ...
      repository.updateStudySession(id, {
        ...progress,
        ...(status ? { status, endedAt: new Date().toISOString() } : {}),
      }),
  });
  ...
  const ensureStudySession = useCallback(async () => {
    ...
    if (pendingSessionBootstrapRef.current) {
      return pendingSessionBootstrapRef.current;
    }

    const pending = createStudySessionMutation
      .mutateAsync()
      .then((created) => {
        if (isMountedRef.current) {
          setStudySessionId(created.id);
          setStudySessionStatus(created.status);
        }
        return created.id;
      })
      .catch((error) => {
      ...
      .finally(() => {
        pendingSessionBootstrapRef.current = null;
      });

    pendingSessionBootstrapRef.current = pending;
    return pending;
  }, [createStudySessionMutation, studySessionId]);

  const persistStudySessionProgress = useCallback(
    async (nextProgress: StudySessionProgress, status?: StudySessionStatus) => {
      const id = await ensureStudySession();
      if (!id) {
        return;
      }
      try {
        const updated = await updateStudySessionMutation.mutateAsync({
          id,
          status,
          progress: nextProgress,
        });
        if (isMountedRef.current) {
          setStudySessionStatus(updated.status);
        }
      } catch (error) {
        console.error("Failed to update study session", error);
      }
    },
    [ensureStudySession, updateStudySessionMutation],
  );

  const handleExit = useCallback(async () => {
    if (studySessionStatus === "active") {
      await persistStudySessionProgress(sessionProgress, "abandoned");
    }
    onExit();
  }, [
    onExit,
  ...
  const handleAnswer = useCallback(
    async (rating: number) => {
      ...
      const nextProgress = nextStudySessionProgress(sessionProgress, rating);
  ...
  useEffect(() => {
    ...
    setStudySessionStatus(null);
    setSessionProgress(EMPTY_STUDY_SESSION_PROGRESS);
    pendingSessionBootstrapRef.current = null;
  }, [deckId]);
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      if (showAnswer && ["1", "2", "3", "4"].includes(event.key)) {
        void handleAnswer(parseInt(event.key));
      }
    };
  ...
  useEffect(() => {
    if (
      ...
      cards.length === 0 ||
      studySessionId ||
      pendingSessionBootstrapRef.current
    ) {
      return;
    }
    void ensureStudySession();
  }, [cards, ensureStudySession, studySessionId]);
      ...
      <PageSection className="p-4 sm:p-5">
        <div className="flex flex-wrap items-center gap-2">
            ...
            {showFlagMenu ? (
              <div className="absolute left-0 top-full z-10 mt-2 min-w-[12rem] overflow-hidden rounded-[1.25rem] border border-[var(--app-line)] bg-[var(--app-panel)] shadow-2xl">
                {FLAG_COLORS.map((flag) => (
                  ...
                  >
                    <span
                      className={`h-3 w-3 rounded-full ${flag.id === 0 ? "border border-[var(--app-line-strong)] bg-transparent" : flag.color.split(" ")[0]}`}
                    />
                    {flag.name}
                  </button>
                ))}
          ...
          >
            <svg
              className="h-4 w-4"
              fill={currentCard.marked ? "currentColor" : "none"}
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
        ...
        <div className="px-4 py-6 sm:px-5 sm:py-8">
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: DOMPurify.sanitize(currentCard.front),
            }}
          />
        </div>
        ...
        <ActionBar>
          <div className="space-y-3">
            ...
            <p className="text-center text-xs text-[var(--app-muted)]">
              Press{" "}
              <kbd className="rounded-lg border border-[var(--app-line-strong)] bg-[var(--app-card)] px-2 py-1">
                Space
              </kbd>{" "}
              or{" "}
              <kbd className="rounded-lg border border-[var(--app-line-strong)] bg-[var(--app-card)] px-2 py-1">
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              {ANSWER_BUTTONS.map((button) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    void handleAnswer(button.rating);
                  }}
