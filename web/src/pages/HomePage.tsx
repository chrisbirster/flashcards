...
function StatCard({
  ...
  return (
    <div className="rounded-3xl border border-[var(--app-line)] bg-[var(--app-card)] p-5 shadow-sm">
      <p className="text-xs uppercase tracking-[0.24em] text-[var(--app-muted)]">{label}</p>
    ...
    <div className="space-y-6">
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,0.9fr)]">
        <div className="rounded-[2rem] bg-[var(--app-card-strong)] px-6 py-8 text-[var(--app-text)] shadow-sm md:px-8 md:py-10">
          <p className="text-xs uppercase tracking-[0.3em] text-[var(--app-accent)]">Workspace overview</p>
        ...
        <div className="rounded-[2rem] border border-[var(--app-line)] bg-[var(--app-card)] p-6 shadow-sm">
          <p className="text-xs uppercase tracking-[0.28em] text-[var(--app-muted)]">Plan usage</p>
          <div className="mt-5 space-y-5">
            ...
            <div className="rounded-2xl bg-[var(--app-muted-surface)] p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-[var(--app-muted)]">Current plan</p>
              <p className="mt-2 text-2xl font-semibold text-[var(--app-text)]">{plan}</p>
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
        <div className="rounded-[2rem] border border-[var(--app-line)] bg-[var(--app-card)] p-6 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-xl font-semibold tracking-tight text-[var(--app-text)]">Recent note activity</h3>
              <p className="mt-1 text-sm text-[var(--app-text-soft)]">Jump back into the notes you created or edited most recently.</p>
            </div>
            <Link to="/notes/view" className="text-sm font-medium text-[var(--app-accent)] hover:brightness-110">
              View all
            </Link>
          </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {note.deckName && (
                          <span className="text-xs uppercase tracking-[0.16em] text-[var(--app-muted)]">{note.deckName}</span>
                        )}
                      </div>
                      <p className="mt-2 truncate text-sm font-medium text-[var(--app-text)]">{note.fieldPreview || 'Untitled note'}</p>
                      {note.tags.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-1">
                          {note.tags.slice(0, 3).map((tag) => (
                            <span key={tag} className="rounded-full bg-[var(--app-card)] px-2 py-1 text-xs text-[var(--app-text-soft)] ring-1 ring-[var(--app-line)]">
                              #{tag}
                            </span>
                          ))}
                        </div>
        ...
        <div className="rounded-[2rem] border border-dashed border-[var(--app-line-strong)] bg-[var(--app-card)] p-6 shadow-sm">
          <p className="text-xs uppercase tracking-[0.28em] text-[var(--app-muted)]">Study Groups</p>
          <h3 className="mt-4 text-2xl font-semibold tracking-tight text-[var(--app-text)]">Deck-linked collaboration is planned next.</h3>
